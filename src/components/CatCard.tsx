...
export function CatCard({ cat, index, depth }: CatCardProps) {
  ...
  return (
    <Reveal as="li" from={index % 2 === 0 ? 'left' : 'right'} delay={index * 0.05} amount={0.3}>
      <motion.div ref={ref} style={{ y }} className="h-full">
        <figure
          className={`group h-full rounded-[26px] bg-white p-3 pb-5 shadow-cute transition-transform duration-200 ease-out hover:-translate-y-2 hover:rotate-0 ${cat.tilt}`}>
